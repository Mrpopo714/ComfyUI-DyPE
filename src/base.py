...
class DyPEBasePosEmbed(nn.Module):
    ...
    """
    def __init__(self, theta: int, axes_dim: list[int], method: str = 'yarn', yarn_alt_scaling: bool = False, dype: bool = True, dype_scale: float = 2.0, dype_exponent: float = 2.0, base_resolution: int = 1024, dype_start_sigma: float = 1.0, base_patch_grid: tuple[int, int] = None, spatial_axes: tuple[int, ...] = None):
        super().__init__()
        self.theta = theta
        ...
        if spatial_axes is None:
            spatial_axes = tuple(i for i in (1, 2) if i < len(self.axes_dim))

        normalized_spatial_axes = []
        seen_axes = set()
        for axis in spatial_axes:
            axis_i = int(axis)
            if axis_i < 0 or axis_i >= len(self.axes_dim) or axis_i in seen_axes:
                continue
            normalized_spatial_axes.append(axis_i)
            seen_axes.add(axis_i)
        self.spatial_axes = tuple(normalized_spatial_axes)

        self._base_axis_len_by_axis = {}
        for i, axis in enumerate(self.spatial_axes):
            if i < len(self.base_patch_grid):
                self._base_axis_len_by_axis[axis] = float(self.base_patch_grid[i])
    ...
    def _base_axis_len(self, axis_index: int) -> float:
        return self._base_axis_len_by_axis.get(axis_index, float(self.base_patches))

    def _spatial_max_patches(self, pos: torch.Tensor) -> float:
        if len(self.spatial_axes) == 0:
            return self._axis_token_span(pos)
        spans = [self._axis_token_span(pos[..., axis]) for axis in self.spatial_axes]
        return max(spans)

    def _spatial_scale_global(self, pos: torch.Tensor) -> float:
        if len(self.spatial_axes) == 0:
            max_current_patches = self._axis_token_span(pos)
            return max(1.0, max_current_patches / self.base_patches)

        axis_scales = []
        for axis in self.spatial_axes:
            current_span = self._axis_token_span(pos[..., axis])
            axis_scales.append(current_span / max(1.0, self._base_axis_len(axis)))
        return max(1.0, max(axis_scales))
    ...
    def _calc_yarn_components(self, pos: torch.Tensor, freqs_dtype: torch.dtype):
        n_axes = pos.shape[-1]
        components = []
        max_current_patches = self._spatial_max_patches(pos)
        ...
        if needs_extrapolation and self.yarn_alt_scaling:
            ...
            if needs_extrapolation and len(self.spatial_axes) > 0:
                spatial_ref_axis = self.spatial_axes[0]
                spatial_axis_dim = self.axes_dim[spatial_ref_axis]
            ...
            for i in range(n_axes):
                ...
                if self._is_spatial_axis(i) and needs_extrapolation and cos_full_spatial is not None:
